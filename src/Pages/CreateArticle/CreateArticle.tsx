...
import {AccountsDataModelsDataModelsTag, ArticleApi} from "@gkju/vlo-boards-client-axios-ts";
...
import {
    ...
    useArticlePicture,
    useArticleTags,
    useAvailableTags, useDeleteTag, useSetAutoPublish,
    useSetContent, useSetPublic, useSetTag,
    useSetTitle
} from "./Queries";
...
export const CreateArticle = () => {
    ...
    const [tagsValues, setTagsValues] = useState<AccountsDataModelsDataModelsTag[]>([]);
    const [autoPublishValue, setAutoPublishValue] = useState('');
    const tagsChangeHandler = (event: React.ChangeEvent<{}>, values: AccountsDataModelsDataModelsTag[]) => {
    ...
    const availableTags: AccountsDataModelsDataModelsTag[] = tags.data?.data ?? [];
    ...
    if(article?.autoPublish && article?.autoPublishOn && +new Date(article.autoPublishOn) > +new Date("0001-01-01T00:00:00") && +new Date(article.autoPublishOn) < +Date.now()) {
        article.public = true;
    }
    ...
    return <motion.div initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}}>
        ...
        <div className="max-h-[200vh]">
            <input type="text" placeholder="Tytuł" value={title} onChange={handleTitleChange} className="p-5 fixed top-0 outline-none text-[150%] bg-[rgb(23,23,32)] w-full z-20" />
                ...
                <motion.div animate={{y: isOpen ? 0 : 300, transition: {duration: 0.3}}} className="pointer-events-auto grid h-[400px] rounded-t-2xl items-center grid-rows-[100px_1fr_1fr_1fr] w-[100vw] grid-cols-5 bg-[rgb(23,23,32)]">
                    <div className="w-full col-span-full text-[rgba(255,255,255,0.3)] p-10 whitespace-nowrap">
                        ...
                        <span className="float-right">
                            {article?.public ? 'Publiczny' : 'Niepubliczny, autopublikacja jest ' + (article?.autoPublish ? 'włączona' : 'wyłączona')}
                        </span>
                    </div>
                    ...
                    <div className="row-start-4 col-start-5 p-10 w-full">
                        <Button
                            onPointerUp={handlePublicChange}
